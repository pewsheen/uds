...
async function main() {
  ...
  for (const box of settings.boxes) {
    const target = decideMountTarget(
      initialMode,
      player.fullscreenEl() !== null,
    );
  ...
  window.addEventListener("message", (e: MessageEvent) => {
    ...
    try {
      ...
      /* opaque provider URL; bridge metadata may still identify it */
    }
    if (!capLang) {
      dlog("caption missing language", d.url);
      return;
    }
    ...
    if (capV && curV && capV !== curV) {
      dlog("caption v", capV, "!=", curV, "skip");
      return;
    }
    ...
    dlog(
      ...
      "cues",
      cues.length,
      matched ? "ASSIGNED" : "DROPPED",
      "| tracks",
      tracks.map((t) => t.languageCode),
      "boxLangs",
      settings.boxes.map((b) => b.lang),
    );
  });
  window.addEventListener("message", (e: MessageEvent) => {
    if (e.source !== window) return;
    const d = e.data as {
      __dualSubsFetchViaExtension?: boolean;
      url?: string;
      languageCode?: string;
      videoId?: string | null;
      asr?: boolean;
  ...
  function render() {
    ...
    renderer.ensureMount(
      decideMountTarget(player.displayMode(), fsEl !== null),
      fsEl,
    );
    ...
    for (const box of settings.boxes) {
      ...
      if (draggingId !== box.id) {
        ...
        views[box.id]!.place(
          { fx: place.fx, fy: place.fy },
          refBoxFor(place.anchor),
          place.vEdge,
          place.anchor,
        );
      }
  ...
  const isStuck = () =>
    settings.boxes.some(
      (b) => pickTrack(tracks, b.lang) && (cuesByBox[b.id]?.length ?? 0) === 0,
    );
  async function startLoading() {
    ...
    for (
      ...
      if (!nudged && gen === loadGen && isStuck()) {
        ...
        const wanted = new Set(
          settings.boxes
            .map((b) => pickTrack(tracks, b.lang)?.languageCode)
            .filter(Boolean),
        );
        const other =
          tracks.find((t) => !wanted.has(t.languageCode)) ??
          tracks.find((t) => t.kind === "asr" && wanted.has(t.languageCode));
        if (other) {
          dlog(
            "startLoading gen",
            gen,
    ...
    dlog(
      ...
      gen,
      "exit cues",
      settings.boxes.map((b) => cuesByBox[b.id]?.length ?? 0),
    );
  }
  ...
  async function loadVideo() {
    ...
    if (myNav !== navGen) {
      dlog("loadVideo nav#", myNav, "superseded");
      return;
    ...
    dlog(
      ...
      myNav,
      "tracks",
      tracks.map((t) => t.languageCode + (t.kind === "asr" ? ":asr" : "")),
    );
    ...
    const end = () => {
      ...
      void (async () => {
        ...
        if (me.anchor === otherPlace.anchor) {
          ...
          const myRect = computeBoxRect(
            { fx: me.fx, fy: me.fy },
            ref,
            view.measure(),
            me.vEdge,
          );
          const otherRect = computeBoxRect(
            { fx: otherPlace.fx, fy: otherPlace.fy },
            ref,
            views[other.id]!.measure(),
            otherPlace.vEdge,
          );
          const cleared = avoidOverlap(myRect, [otherRect], 8);
          me.fy = toFraction(
            {
              x: cleared.x + cleared.width / 2,
              y: me.vEdge === "top" ? cleared.y : cleared.y + cleared.height,
            },
            ref,
          ).fy;
        }
