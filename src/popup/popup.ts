...
function fillSelect(
  ...
  for (const t of tracks) {
    ...
    sel.add(
      new Option(t.name ? `${t.name} (${t.languageCode})` : value, value),
    );
    seen.add(value);
  }
  if (current && !seen.has(current))
    sel.add(new Option(`${current} (not on this video)`, current));
  sel.value = current;
}
...
async function init() {
  ...
  native.addEventListener("change", () => {
    s.nativeSubtitles = native.checked;
    scheduleSave();
  });
  ...
  s.boxes.forEach((box, i) => {
    ...
    bgColor.addEventListener("input", () => {
      box.style.bgColor = bgColor.value;
      scheduleSave();
    });
    ...
    size.addEventListener("change", () => {
      box.style.fontSizePx = parseInt(size.value, 10) || 24;
      scheduleSave();
    });
    outline.addEventListener("change", () => {
      box.style.outline = outline.checked;
      scheduleSave();
    });
  });
}
