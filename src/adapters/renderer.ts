...
export function createRenderer(doc: Document = document) {
  ...
  function createBox(target: MountTarget, fullscreenEl: Element | null, id: string): BoxView {
    ...
    el.dataset.boxId = id
    el.style.cssText = 'position:fixed;max-width:80vw;text-align:center;padding:6px 12px;border-radius:10px;pointer-events:auto;cursor:grab;white-space:normal;user-select:none;-webkit-user-select:none;touch-action:none;'
    root.appendChild(el)
    return {
      el,
  ...
  return {
    ...
    hideGlow: () => { if (glow) glow.style.display = 'none' },
    destroy: () => { layer?.remove(); layer = null; glow = null },
  }
}
