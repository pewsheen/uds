...
function videoScore(video: HTMLVideoElement): number {
  ...
  return (
    (video.classList.contains("html5-main-video") ? 1000 : 0) +
...
export function createPlayerAdapter(doc: Document = document): Player {
  ...
  const videos = () =>
    Array.from(
      doc.querySelectorAll<HTMLVideoElement>("video.html5-main-video, video"),
    );
  const findPrimeVideo = () =>
    bestBy(videos().filter(isPrimePlaybackVideo), videoScore);
  const findVideo = () =>
    prime ? findPrimeVideo() : bestBy(videos(), videoScore);
  const findVisibleVideo = () => bestBy(videos(), areaOf);
  const findPlayer = () =>
    doc.querySelector(
      '#movie_player, .html5-video-player, #dv-web-player, [data-testid="web-player"], [class*="webPlayer" i]',
    );
  return {
    video: findVideo,
    playerRect: () => {
    ...
    displayMode: () => {
      ...
      const body = doc.body;
      if (
        body.classList.contains("ytd-miniplayer") ||
        doc.querySelector("ytd-miniplayer[active]")
      )
        return "miniplayer";
