...
import { extractPrimeTracks, isPrimeCaptionUrl, looksLikePrimeCaptionBody } from '../core/prime'
import type { CaptionTrackRaw, LoadRequest, ProviderHooks, SiteProviderFactory } from './types'

const RESPONSE_HINT = /(?:getplaybackresources|playback|subtitle|caption|timedtext|ttml|webvtt|dfxp|\.vtt|\.ttml|\.ttml2)/i
...
function createPrimeVideoProvider(hooks: ProviderHooks) {
  ...
  function setTracks(nextTracks: CaptionTrackRaw[]): void {
    const key = JSON.stringify(nextTracks.map((t) => [t.languageCode, t.kind ?? '', t.baseUrl, t.name?.simpleText ?? '']))
    if (key === tracksKey) return
    tracksKey = key
    tracks = nextTracks
    if (publishSnapshot()) {
      hooks.debug('prime tracks', tracks.map((t) => `${t.languageCode}:${t.name?.simpleText ?? ''}`))
  ...
  function pickTrack(request: LoadRequest): CaptionTrackRaw | undefined {
    ...
    const exact = tracks.filter((track) => track.languageCode.toLowerCase() === lang)
    const primary = lang.split('-')[0]
    const pool = exact.length ? exact : tracks.filter((track) => track.languageCode.toLowerCase().split('-')[0] === primary)
    if (!pool.length) return undefined
    return pool.find((track) => (track.kind === 'asr') === !!request.asr) ?? pool[0]
  }
...
function sameUrl(a: string, b: string): boolean {
  if (a === b) return true
  try { return new URL(a, location.href).href === new URL(b, location.href).href } catch { return false }
}
