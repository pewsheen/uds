...
function createYouTubeProvider(hooks: ProviderHooks) {
  function currentPlayerResponse(): PlayerResponse {
    ...
    return (fromApi ??
      (window as unknown as { ytInitialPlayerResponse?: unknown })
        .ytInitialPlayerResponse) as PlayerResponse;
  }
  ...
  function load(request: LoadRequest): void {
    ...
    const player = document.getElementById(
      "movie_player",
    ) as unknown as YtPlayer | null;
    if (!player) return;
    try {
      player.loadModule?.("captions");
      const response = currentPlayerResponse() as
        | {
            captions?: {
      ...
      const list = ((responseTracks && responseTracks.length
        ? responseTracks
        : player.getOption?.("captions", "tracklist", { includeAsr: true })) ??
        []) as { languageCode?: string; kind?: string }[];
  ...
  return {
    ...
    start: () => {
      ...
      document.addEventListener("yt-navigate-finish", () => {
        hooks.debug(
          "youtube nav-finish: republish wantId",
          parseWatchId(location.href),
        );
        publishCurrent(parseWatchId(location.href));
      });
    },
