// YouTube is a single-page app: it swaps videos via its Polymer router without a
// document reload, so the extension must detect navigation itself. These pure helpers
// turn a URL into a stable "which video is this?" id and decide when that id changed
// in a way that should trigger per-video re-initialization.

/** The video id for a YouTube URL, or null when the URL is not a single video page. */
