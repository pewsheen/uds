...
export function parseSelector(sel: string): { lang: string; asr: boolean } {
  if (sel.startsWith('asr:')) return { lang: sel.slice(4).toLowerCase(), asr: true }
  return { lang: sel.toLowerCase(), asr: false }
}

export function selectorForTrack(t: { languageCode: string; kind?: string }): string {
...
export function captureMatchesBox(selector: string, want: CaptionTrack, capLang: string, capAsr: boolean): boolean {
...
export function pickTrack(tracks: CaptionTrack[], sel: string): CaptionTrack | null {
  if (!sel) return null
  const { lang, asr } = parseSelector(sel)
  const exact = tracks.filter((t) => t.languageCode.toLowerCase() === lang)
  const primary = lang.split('-')[0]
  const pool = exact.length ? exact : tracks.filter((t) => t.languageCode.toLowerCase().split('-')[0] === primary)
