...
const JSON3 = JSON.stringify({ events: [{ tStartMs: 0, dDurationMs: 600000, segs: [{ utf8: 'asr probe' }] }] })
...
try {
  ...
  await page.addInitScript(() => {
    window.__tt = []
    const rec = (u) => { if (String(u).includes('/api/timedtext')) window.__tt.push(String(u)) }
    const of = window.fetch
    window.fetch = function (...a) { rec(a[0]?.url ?? a[0]); return of.apply(this, a) }
    const oo = XMLHttpRequest.prototype.open
    XMLHttpRequest.prototype.open = function (m, u, ...r) { rec(u); return oo.call(this, m, u, ...r) }
  })
  ...
  const tracklist = await page.evaluate(() => {
    ...
    return list.map((t) => ({ lang: t.languageCode, kind: t.kind || '', name: t.name?.simpleText }))
  })
  ...
  const parse = (u) => { const q = new URL(u).searchParams; return { lang: q.get('lang'), kind: q.get('kind') } }
