...
const posByMode = Object.fromEntries(
  MODES.map((m) => [m, { anchor: "video", vEdge: "bottom", fx: 0.5, fy: 0.9 }]),
);
const SETTINGS = {
  enabled: true,
  nativeSubtitles: false,
  boxes: [
    { id: "sub1", lang: "en", style: STYLE, posByMode },
    { id: "sub2", lang: "", style: STYLE, posByMode },
  ],
};
const extId = (p) => {
  ...
  let id = "";
  for (let i = 0; i < 32; i++)
    id += String.fromCharCode(97 + parseInt(h[i], 16));
  return id;
};
...
page.on("console", (m) => {
  const t = m.text();
  if (t.startsWith("[uds.")) uds.push(t);
});
...
const nudgePlay = () =>
  page
    ...
    .catch(() => {});
async function box0Fills(ms) {
  const s = Date.now();
  while (Date.now() - s < ms) {
    await skipAds();
    await nudgePlay();
    if ((await box0()).includes(`v=${ID}`)) return true;
...
try {
  ...
  await pop.evaluate(
    (s) => chrome.storage.sync.set({ dualSubsSettings: s }),
    SETTINGS,
  );
  await pop.close();
  ...
  await ctx.route("**/api/timedtext**", (r) => {
    ...
    r.fulfill({
      ...
      body: json3(u.searchParams.get("v"), lang),
    });
  });
  ...
  try {
    const r = page
      ...
      .first();
    if (await r.isVisible({ timeout: 3000 })) await r.click();
  } catch {}
  ...
  if (!filled) {
    ...
    log(
      "box0 after manual toggle: filled=",
      afterToggle,
      JSON.stringify(await box0()),
    );
  }
  ...
  log(
    "\n" +
      (filled
        ? "✅ box0 fills despite empty first fetch"
        : afterToggle
          ? "❌ REPRODUCED: empty at load, only fills after manual toggle"
