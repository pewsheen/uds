...
const json3 = (label) =>
  JSON.stringify({
    events: [{ tStartMs: 0, dDurationMs: 600000, segs: [{ utf8: label }] }],
  });
  ...
  ctx
    ...
    .evaluate(() => {
      ...
      const adShowing = document.querySelector(
        ".ad-showing, .ytp-ad-player-overlay, .ytp-ad-player-overlay-layout",
      );
      if (adShowing) {
        const v = document.querySelector("video");
        if (v && isFinite(v.duration) && v.duration > 0)
          v.currentTime = v.duration; // fast-forward the ad
        return "ad-playing";
      }
      return null;
    })
...
try {
  ...
  await ctx.route("**/api/timedtext**", (route) => {
    ...
    const lang =
      u.searchParams.get("tlang") || u.searchParams.get("lang") || "?";
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: json3(`CUE[v=${v} ${lang}]`),
    });
  });
  ...
  log(
    "VIDEO A:",
    JSON.stringify({ idA: prevId, boxes: await boxText() }, null, 2),
  );
  ...
  async function spaNavigateAwayFrom(cur) {
    ...
    return waitFor(
      ...
      "a visible recommended video link",
      15000,
    );
  }
  ...
  for (let hop = 1; hop <= 4 && !captionsReloaded; hop++) {
    await page.evaluate(() => {
      window.__spaMarker = "kept";
    }); // wiped by a full reload, kept by SPA
    ...
    const attr = await waitFor(
      async () => ((await videoIdOfAttr()) === id ? id : null),
      "bridge attr republish",
    );
    const clearedOld = !(await boxText()).some(
      (t) => t && t.includes(`v=${prevId}`),
    );
    let filled = false;
    try {
      await waitFor(
        async () =>
          (await boxText()).some((t) => t && t.includes(`v=${id}`))
            ? true
            : null,
        "captions reload",
        14000,
    ...
    log(
      `  wasSpa=${wasSpa} attrRepublished=${attr === id} clearedOldCues=${clearedOld} captionsReloaded=${filled} boxes=${JSON.stringify(await boxText())}`,
    );
