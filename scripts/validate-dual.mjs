...
const VIDEO =
  process.env.VIDEO_URL || "https://www.youtube.com/watch?v=aircAruvnKk";
...
try {
  ...
  try {
    const r = page
      ...
      .first();
    if (await r.isVisible({ timeout: 2500 })) {
      await r.click();
  ...
  log(
    filled.length >= 2
      ? "✅ PASS — both boxes filled (dual-language sequential load works)"
