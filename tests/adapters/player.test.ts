...
function fakeDocument(videoEls: HTMLVideoElement[], hostname = 'www.youtube.com', playerEl: Element | null = null): Document {
  return {
    body: { classList: { contains: () => false } },
    fullscreenElement: null,
    location: { hostname },
    querySelector: (selector: string) => selector.includes('player') ? playerEl : null,
...
test('waits for a visible Prime playback timeline instead of the detail-page preview', () => {
  ...
  const player = createPlayerAdapter(fakeDocument([preview, hiddenPlayback], 'www.primevideo.com'))
...
test('uses the visible Prime playback video once the player is active', () => {
  ...
  const player = createPlayerAdapter(fakeDocument([playback], 'www.primevideo.com', fullscreenPlayer))

  expect(player.clockReady()).toBe(true)
  expect(player.currentTime()).toBe(632.849)
  expect(player.playerRect()).toEqual({ x: 40, y: 25, width: 1100, height: 630 })
})
