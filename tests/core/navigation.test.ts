...
test("parseWatchId reads the v param from a watch URL", () => {
  ...
  expect(parseWatchId("https://www.youtube.com/watch?v=abc123&t=42s")).toBe(
    "abc123",
  );
  expect(parseWatchId("https://www.youtube.com/watch?list=PL1&v=xY_-9z")).toBe(
    "xY_-9z",
  );
});
...
test("parseWatchId returns null off a watch page", () => {
  ...
  expect(
    parseWatchId("https://www.youtube.com/results?search_query=cats"),
  ).toBeNull();
...
test("parseWatchId reads Prime Video detail ids", () => {
  expect(
    parseWatchId(
      "https://www.primevideo.com/detail/0SS3O1MC4E0TW4GF0V1E4KD06T",
    ),
  ).toBe("0SS3O1MC4E0TW4GF0V1E4KD06T");
  expect(
    parseWatchId(
      "https://www.primevideo.com/region/fe/detail/0SS3O1MC4E0TW4GF0V1E4KD06T?ref_=atv_plr_landingpage_play",
    ),
  ).toBe("0SS3O1MC4E0TW4GF0V1E4KD06T");
  expect(
    parseWatchId(
      "https://www.primevideo.com/gp/video/detail/B0ABCDE123/ref=atv_dp",
    ),
  ).toBe("B0ABCDE123");
});
