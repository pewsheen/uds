...
import {
  ...
  selectorForTrack,
  captureMatchesBox,
} from "../../src/core/track-select";

const T = (languageCode: string, name?: string, kind?: string) => ({
  baseUrl: `https://yt/${languageCode}${kind ?? ""}`,
  languageCode,
  name,
  kind,
});
...
test("exact language match wins", () => {
  expect(pickTrack([T("en"), T("zh-Hant")], "zh-Hant")?.languageCode).toBe(
    "zh-Hant",
  );
});
...
test("asr: selector picks the auto-generated track", () => {
  const tracks = [
    T("en", "English"),
    T("en", "English (auto-generated)", "asr"),
  ];
...
test("asr: selector requires the auto track and rejects the human capture", () => {
  expect(
    captureMatchesBox("asr:en", T("en", undefined, "asr"), "en", true),
  ).toBe(true);
  expect(
    captureMatchesBox("asr:en", T("en", undefined, "asr"), "en", false),
  ).toBe(false);
});
...
test("kind handling is language-agnostic — asr works for any language, not just en", () => {
  ...
  expect(
    captureMatchesBox("asr:ja", T("ja", undefined, "asr"), "ja", true),
  ).toBe(true);
  expect(
    captureMatchesBox("asr:ja", T("ja", undefined, "asr"), "ja", false),
  ).toBe(false);
